            ...
            if (($message_data['model'] == 'gateway' || $message_data['model'] == 'acpartner.v3') && $message_data['cmd'] == 'heartbeat') {
                ...
                if ($message_data['cmd'] == 'report' || $message_data['cmd'] == 'read_ack' || $message_data['cmd'] == 'read_rsp') {
                    ...
                    if (isset($message_data['data']['pressure'])) {
                        ...
                        $original_value = ((float)$message_data['data']['pressure']) / 1000;
                        $value = round($original_value, (int)$roundMode);
                        $got_commands[] = array('command' => $command, 'value' => $value);

                        $command = 'pressure_mm';
                        $value = round($original_value * 7.50062, (int)$roundMode);
                        $got_commands[] = array('command' => $command, 'value' => $value);
                    }
    ...
    function usual(&$out)
    {
        if ($this->ajax) {
            ...
            if ($op == 'process') {
                $message = gr('message');
                $ip = gr('ip');
                $log_debmes = gr('log_debmes');
                $log_gw_heartbeat = gr('log_gw_heartbeat');
